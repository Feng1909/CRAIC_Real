...
bool TrajOpt::generate_traj(const Eigen::MatrixXd& iniState,
  ...
  if (opt_once) {
    ...
    tail_f = init_tail_f_;
    vt = init_vt_;
    std::cout<<"once planning..."<<std::endl;
  } else {
    ...
    do {
      ...
      point_now(0) = car_p_(1) - cos(time_now) + cos(T_bvp/3);
      point_now(1) = car_p_(1) - sin(time_now) + sin(T_bvp/3);
      point_now(2) = car_p_(2);
      bvp_f.col(0) = point_now;
