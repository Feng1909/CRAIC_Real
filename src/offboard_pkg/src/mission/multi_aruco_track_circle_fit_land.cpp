...
int main(int argc, char **argv)
{
    ...
    ros::Publisher setpoint_raw_local_pub = nh.advertise<mavros_msgs::PositionTarget>("/aruco_pose_mavros", 10);
...
std::cout<<"state flag"<<state_flag<<std::endl;
    //起飞
    if(state_flag == 0)
    {
    ...
    setpoint_raw_local_pub.publish(pos_setpoint);
      ...
      if((abs(cur_position[0]) < 0.1)&&(abs(cur_position[1] + 1) < 0.1)&&((abs(cur_position[2]-0.7)) < 0.1)&&(ros::Time::now() - time_snap0 > ros::Duration(10.0)))
      {
       //state_flag = 1;
       state_flag = 1;
      }
