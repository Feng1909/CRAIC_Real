...
void pos_cb(const nav_msgs::Odometry::ConstPtr &msg)
        {
            cur_position[0] = msg->pose.pose.position.x;
            cur_position[1] = msg->pose.pose.position.y;
            cur_position[2] = msg->pose.pose.position.z;
    
            //排序是 roll,pitch,yaw
            Eigen::Quaterniond q_fcu = Eigen::Quaterniond(msg->pose.pose.orientation.w, msg->pose.pose.orientation.x, msg->pose.pose.orientation.y, msg->pose.pose.orientation.z);
...
void aruco_state_cb(const std_msgs::Bool::ConstPtr &msgs) {
  detected_1 = msgs->data;
}

void state_machine_cb(const std_msgs::Int32::ConstPtr &msgs) {
  state_machine = msgs->data;
}

void multi_aruco_det_cb(const geometry_msgs::PoseStamped::ConstPtr &msg)
        {   
            // detected_1=msg->detected;
            // position_det[0] = msg->position[0];
            // position_det[1] = msg->position[1];
            // position_det[2] = msg->position[2];
            position_det[0] = msg->pose.position.x;
            position_det[1] = msg->pose.position.y;
            position_det[2] = msg->pose.position.z;
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber position_sub = nh.subscribe<nav_msgs::Odometry>("/vins_fusion/imu_propagate", 10, pos_cb);
    ...
    ros::Subscriber multi_aruco_det_sub = nh.subscribe<geometry_msgs::PoseStamped>("/aruco_det/target_loc", 10, multi_aruco_det_cb);
    ros::Subscriber aruco_state_sub = nh.subscribe<std_msgs::Bool>("/aruco_det/state", 10, aruco_state_cb);

    ros::Subscriber state_machine_sub = nh.subscribe<std_msgs::Int32>("/state_machine", 10, state_machine_cb);
